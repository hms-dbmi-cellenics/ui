...
import loadDownsampledGeneExpression from 'redux/actions/genes/loadDownsampledGeneExpression';
import {
...
describe('loadDownsampledGeneExpression Redux action - expression matrix data check', () => {
  let store;
  ...
  it('does not make a work request when all genes are already loaded in the matrix', async () => {
    ...
    await store.dispatch(loadDownsampledGeneExpression(
      'exp123',
    ...
    // Should still dispatch LOADING then LOADED for UI consistency
    expect(dispatchedActions).toHaveLength(2);
    expect(dispatchedActions[0].type).toBe(DOWNSAMPLED_GENES_EXPRESSION_LOADING);
    expect(dispatchedActions[1].type).toBe(DOWNSAMPLED_GENES_EXPRESSION_LOADED);
  });
