...
describe('loadEmbedding action', () => {
  ...
  it('loadEmbedding generates correct hash params body for ETag', async () => {
    ...
    await store.dispatch(loadEmbedding(
      experimentId,
      experimentSettings.processing.configureEmbedding.embeddingSettings.method,
    ));
    expect(hashMock).toHaveBeenCalled();
    // this snapshot should match exactly API snapshot:
    // submitMarkerHeatmapWork.test.js.snap
    expect(hashMock.mock.calls).toMatchSnapshot('embeddings - hash call params');
