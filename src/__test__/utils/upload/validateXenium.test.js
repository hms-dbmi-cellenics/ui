...
const makeSample = () => ({
  files: {
    ...
    [sampleFileType.XENIUM_TRANSCRIPTS]: { fileObject: makeParquet() },
  },
});
...
describe('validateXenium', () => {
  ...
  it('throws when the required transcripts.parquet is absent', async () => {
    const sample = makeSample();
    delete sample.files[sampleFileType.XENIUM_TRANSCRIPTS];
    await expect(validateXenium(sample)).rejects.toThrow(/transcripts\.parquet/);
  });

  it('throws when transcripts.parquet is not valid Parquet', async () => {
    const sample = makeSample();
